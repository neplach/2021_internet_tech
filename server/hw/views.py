...
class SecondViewSet(viewsets.GenericViewSet):
    ...
    def partial_update(self, request, pk=None):
        first = get_object_or_404(self.get_queryset(), id=pk)
        serializer = self.get_serializer(first, data=request.data, partial=True)
        if request.data.get('fk', None):
            objects = Second.objects.filter(fk=request.data.get('fk'))
            first_object = First.objects.filter(id=request.data.get('fk'))[0]
            if len(objects) >= first_object.max:
